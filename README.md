...
See `tar_git`'s [interface definition](https://github.com/MeeGoIntegration/obs-service-tar-git/blob/master/tar_git.service).
...
4. If there is no valid tag or if a commit ID (short-hash) was provided, `tar_git` [looks for the "closest" tag](https://github.com/MeeGoIntegration/obs-service-tar-git/blob/master/tar_git#L960) which fulfils aforementioned rules (i.e., runs through steps 1 to 3 with it), but misses to filter for the branch provided by the `branch` parameter.  It then appends a date string and a short-hash, which are a clear indication that this code path was used.
